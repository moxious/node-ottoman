...
function _encodeValue(context, type, value, forceTyping, field) {
  if (context.isModel(type)) {
    if (!(value instanceof type)) {
      throw new Error('Expected field ' + field.name + ' type to be a `' + type.name + '`');
    }
    return value._toCoo(type.name, forceTyping);
  } else if (type instanceof Schema.ListField) {
    if (!Array.isArray(value)) {
      throw new Error('Expected field ' + field.name + ' type to be an array.');
    }
    var outArr = [];
    for (var i = 0; i < value.length; ++i) {
      outArr[i] = _encodeValue(context, type.type, value[i], forceTyping, field);
    }
    return outArr;
  } else if (type instanceof Schema.FieldGroup) {
    if (!(value instanceof Object)) {
      throw new Error('Expected field ' + field.name + ' object type but got non-object.');
    }
    var outObj = {};
    for (var j in value) {
      ...
      if (value.hasOwnProperty(j)) {
        ...
        outObj[j] = _encodeValue(context, field.type, value[j], forceTyping,field);
      }
    }
    return outObj;
...
ModelInstance.prototype._toCoo = function(refType, forceTyping) {
  ...
  for (var i in this) {
    ...
    if (this.hasOwnProperty(i)) {
      ...
      objOut[i] =
          _encodeValue($.schema.context, field.type, this[i], forceTyping,field);
    }
  }
  return objOut;
};
